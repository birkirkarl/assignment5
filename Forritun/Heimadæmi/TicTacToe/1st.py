...
def left_slope_victory(bordid,vidd):
    win_list=[]
    for i in range(vidd): #skácheckari frá vinstra efra horni
        win_list.append(bordid[i][i])
    sigur=last_check_if_victory(win_list)
    return sigur

def right_slope_victory(bordid,vidd):
    win_list=[]
    count=2
    for i in range(vidd):
...
player2=False
sigur=True
vidd=int(input("Input the dimension of the board: "))
